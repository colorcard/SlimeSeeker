...
后台完成挂机点扫描。运行状态将第一阶段进度与最终 Top-K 挂机点进度分别存储，界面不会混用
吞吐、ETA 或分母。群系结果页按评分所用的相同区块中心预筛和逐方块三维距离条件，生成
...
TUI 用显式的可见组件顺序处理 Tab/Shift-Tab，避免 Toggle、结果 Menu 或隐藏的模式字段截获
焦点；模式、保留策略、页面和对话框变化时动态重建焦点顺序。输入校验仍位于无界面模型中，
界面只负责把错误映射回字段。对话框路径输入、覆盖确认、后台导出和完成状态拥有各自的焦点
集合，覆盖确认中的路径为只读展示，不能绕过重新检查。
