...
Minecraft 26.2 群系重评分是 CLI 私有的可选第二阶段，不进入公共 ABI 或搜索 engine。其纯 C++
...
TUI 默认用有界 Top-K 队列保留最佳结果，也允许用户明确选择全量模式。CSV 导出使用独立线程
读取已经不可变的结果数组，通过同目录临时文件和备份替换目标，因而不会阻塞界面或把取消的
写入伪装成完整文件。FTXUI 只私有链接可执行程序，不改变公共库 ABI、安装目标或依赖方向。
