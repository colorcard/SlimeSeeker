...
SlimeSeeker 是一个跨平台的 Minecraft Java Edition 史莱姆农场选址工具、搜索库与命令行程序。给定世界种子，它会在指定区块坐标范围内搜索 17×17 圆环中史莱姆区块数量达到阈值的中心点，并可进一步按 Minecraft 26.2 主世界群系刷怪表重评分、寻找候选中心区块内的挂机位置。
...
TUI 配置页可以在“区块密度”和“26.2 群系评分”间切换，并提供 seed、搜索范围、阈值、线程
...
3. 面向空置域切门农场时切换到“26.2 群系评分”，设置实际生成层脚部 Y 和计划中的玩家脚部 Y。
...
| 26.2 群系评分 | 密度达标后，哪些位置的群系刷怪表更偏向史莱姆，其 Top-K 候选的最佳挂机覆盖是多少？ | 空置域、单层地狱门切门农场精筛 |
...
Minecraft 26.2 中具有相同 MONSTER 刷怪表的群系会被归并为一个权重档。对某个方块所属档位，
工具使用以下期望权重：
...
其中 `monster_weight` 是该群系 MONSTER 列表所有条目权重之和；`slime_group_weight` 是史莱姆
...
第一阶段每个史莱姆区块的 256 个方块都会在生成脚部 Y 采样群系；所有方块权重求和后除以
