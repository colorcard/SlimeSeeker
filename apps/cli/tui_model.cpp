...
std::string_view localized_text(Language language, TextKey key) {
    ...
    static constexpr std::string_view zh[] = {
        ...
        "详情", "自动 CPU", "标量", "AVX2", "NEON", "CUDA", "最佳结果"
    };
...
ValidationResult validate_search_form(const SearchForm &form) {
    ...
    if (form.retention < 0 || form.retention > 1 ||
        (retention == ResultRetention::top &&
         (!parse_integer(form.top_k, top_k) || top_k == 0 || top_k > 1000000))) {
        result.error = ValidationError::top_k; return result;
    }
    ...
    result.request.retention = retention;
    result.request.top_k = retention == ResultRetention::top ? static_cast<size_t>(top_k) : 0;
...
std::string default_export_filename(int64_t seed, bool partial) {
    return "slimeseeker-seed-" + std::to_string(seed) + (partial ? "-partial.csv" : "-results.csv");
}
